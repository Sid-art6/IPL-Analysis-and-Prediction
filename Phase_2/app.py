...
with st.sidebar:
    ...
    **Precision:** 81.1%  
    ...
    [📄 Phase 1 Analysis](https://github.com/Sid-art6/IPL-Analysis-and-Prediction/tree/main/Phase_1)
    
    [📂 Phase 2 Prediction](https://github.com/Sid-art6/IPL-Analysis-and-Prediction/tree/main/Phase_2) 
    """)
...
with col3:
    ...
    is_boundary = st.selectbox("🏏 Current Delivery", ball_options, index=preset["ball"])
...
rr_gap = np.clip(required_rr - current_rr, -30, 30)
...
is_boundary_val = 1 if is_boundary == "Boundary (4 or 6)" else 0
is_dot_val = 1 if is_boundary == "Dot Ball (0 runs)" else 0

features = np.array([[
    runs_scored, runs_needed, current_rr, wickets_in_hand,
    balls_remaining, rr_gap, is_boundary_val, is_dot_val,
    phase_num, last_6_runs
]])
...
if balls_remaining <= 0:
    ...
    def predict_scenario(cum_r, r_need, c_rr, w_hand, b_rem, rr_g, is_b, is_d, ph, mom):
        f = np.array([[cum_r, r_need, c_rr, w_hand, b_rem, rr_g, is_b, is_d, ph, mom]])
        return model.predict_proba(f)[0][1] * 100
    
    # Scenario: Dot ball
    dot_prob = predict_scenario(runs_scored, runs_needed, current_rr, wickets_in_hand,
                                balls_remaining - 1,
                                np.clip((runs_needed) / ((balls_remaining - 1) / 6) - current_rr, -30, 30) if balls_remaining > 1 else 30,
                                0, 1, phase_num, max(last_6_runs - 2, 0))
    
    # Scenario: Boundary (4 runs)
    new_runs_b = runs_scored + 4
    new_need_b = runs_needed - 4
    new_cr_b = new_runs_b / (balls_bowled / 6) if balls_bowled > 0 else 0
    boundary_prob = predict_scenario(new_runs_b, new_need_b, new_cr_b, wickets_in_hand,
                                      balls_remaining - 1,
                                      np.clip((new_need_b) / ((balls_remaining - 1) / 6) - new_cr_b, -30, 30) if balls_remaining > 1 else -30,
                                      1, 0, phase_num, last_6_runs + 4)
    
    # Scenario: Wicket falls
    wicket_prob = predict_scenario(runs_scored, runs_needed, current_rr, max(wickets_in_hand - 1, 1),
                                    balls_remaining - 1,
                                    np.clip((runs_needed) / ((balls_remaining - 1) / 6) - current_rr, -30, 30) if balls_remaining > 1 else 30,
                                    0, 1, phase_num, max(last_6_runs - 2, 0))
    ...
    for future_over in range(current_over, 21):
        future_balls_bowled = future_over * 6
        future_balls_remaining = 120 - future_balls_bowled
        if future_balls_remaining <= 0:
            break
        
        # Project runs scored at current rate
        extra_overs = future_over - current_over
        projected_runs = runs_scored + (current_rr * extra_overs) if current_rr > 0 else runs_scored
        projected_runs_needed = max(target - projected_runs, 0)
        ...
        proj_cr = projected_runs / (future_balls_bowled / 6) if future_balls_bowled > 0 else 0
        proj_rr = projected_runs_needed / (future_balls_remaining / 6) if future_balls_remaining > 0 else 999
        proj_rr_gap = np.clip(proj_rr - proj_cr, -30, 30)
        
        proj_phase = 1 if future_over <= 6 else (2 if future_over <= 15 else 3)
        
        f = np.array([[projected_runs, projected_runs_needed, proj_cr, wickets_in_hand,
                       future_balls_remaining, proj_rr_gap, 0, 0, proj_phase, last_6_runs]])
        prob = model.predict_proba(f)[0][1] * 100
...
<div class="footer">
    ...
    Built by <strong>Ravneet Kaur</strong> for the <a href="https://github.com/SahilGogna/IPL-Hackathon">ORU IPL Analytics Hackathon</a></p>
</div>
""", unsafe_allow_html=True)